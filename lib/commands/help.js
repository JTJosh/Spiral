...
exports.exec = async (msg, {send, Discord, client, member, prefix}) =>{
  ...
  embed.setTitle('**Help**').setDescription(`Spiral's help page.`);
  embed.addField('**Fun**','`8ball`, `Banned`, `Dreams`, `Warm`.');
  embed.addField('**Moderation (Work In Progress!)**','`Ban`, `Kick`, `Mute`, `Clear`.');
  embed.addField('**Utility**','`Ping`, `Info user`, `Info Server`.');
  embed.addField('**Miscellaneous**','`Status`, `Invite`, `Willy`.');
  embed.setThumbnail(myavatar);
  embed.setFooter(`Requested by ${msg.author.tag} | Help`, myauthor);
